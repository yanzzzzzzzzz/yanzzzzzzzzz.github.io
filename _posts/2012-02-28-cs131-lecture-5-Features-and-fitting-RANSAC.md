...
* $$w^n$$是所有$$n$$個點均為是inliers的機率
* $$1-w^n$$是所有$$n$$個點有一個是outliers的機率
* 迭代$$k$$次都沒辦法找到所有點是inliers的機率$$(1-w^n)^k$$
* 迭代$$k$$次所有點是inliers的機率$$1-(1-w^n)^k$$
...
改善RANSAC效率方法：
* 先對資料集進行最小二乘法得到不錯的模型(全局最佳化)，再進行RANSAC(本地最佳化)
...
* 優點
    ...
    * 好實現
* 缺點
    * 對於資料集中outliers數量變多時，時間成本會大幅提升，真實問題通常都有較大佔比的outliers(可能的解決方法：隨機選擇資料集中的子集合)
