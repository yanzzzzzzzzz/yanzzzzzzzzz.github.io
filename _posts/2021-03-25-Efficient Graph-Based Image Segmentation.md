...
3. $$開始分割動作，初始分割區域為S^0, 其中每個像素點v_i是獨立的一個區域$$
4. $$重複步驟5並定義q=1,...m, S^q的初始區域是由S^{q-1}組成, 定義v_i, v_j 是一組前q小的edge權重對應的點對o_q = (v_i, v_j) $$
$$檢查點對合成條件是否符合, 合成條件參考D(C1, C2), 符合則合併, 否則維持分割結果S並繼續執行到結束$$
